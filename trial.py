...
bg = pygame.image.load("image/GARDENN.png").convert()
...
# Load different enemy images for different levels
enemy_images = {
    1: pygame.image.load("z_img/ZOMBIE.png").convert_alpha(),
    2: pygame.image.load("enemy1.png").convert_alpha(),
    3: pygame.image.load("z_img/ZOMBIE.png").convert_alpha()
}

class Enemy:
    def __init__(self, x, y, level):
        self.image = pygame.transform.scale(enemy_images[level], (80, 80))  
...
def spawn_enemy():
    ...
    if side == "left":
        ...
        x = random.randint(0, SCREEN_WIDTH)
        y = SCREEN_HEIGHT
    enemies.append(Enemy(x, y, level))  # Pass the level as an argument here
...
while running:
    ...
    if not game_over:
        ...
        # Adjust spawn interval and number of enemies based on level
        if level == 1:
            enemy_spawn_interval = 180
            max_enemies = 5
        elif level == 2:
        ...
        # Increase level when a certain condition is met (e.g., defeating a certain number of enemies)
        if len(enemies) == 0 and level < 3:
            level += 1
            # Reset player's health for the next level
            player.health = player.max_health
            # Reset enemy spawn timer for the next level
            enemy_spawn_timer = 1
            # Spawn enemies for the next level
            for _ in range(3 + (level - 1) * 5):  # Adjust the number of enemies per level
                spawn_enemy()
